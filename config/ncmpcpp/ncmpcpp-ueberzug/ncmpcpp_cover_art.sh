...
find_cover_image() {
    ...
    if [ "$ext" = "flac" ]; then
        ...
        ffmpeg -v 0 -y -i "$(mpc --format "$music_library"/%file% | head -n 1)" \
            /tmp/mpd_cover.jpg && 
            cover_path="/tmp/mpd_cover.jpg" && return
    fi
    ...
    found_covers="$(find "$album_dir" -maxdepth 1 -type f \
	    -iregex ".*/.*$(basename "$file" | sed 's/\..*$//' | sed -e 's/[]\/$*.^[]/\\&/g').*[.]\(jpe?g\|png\|gif\|bmp\)")"
    cover_path="$(echo "$found_covers" | head -n1)"
    cp "$cover_path" /tmp/mpd_cover.jpg
